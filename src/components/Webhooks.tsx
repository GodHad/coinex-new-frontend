                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            <h2 className="text-lg font-medium">
                                {user?.userMode === 'free' ? 'Free Plan' : user?.userMode === 'standard' ? 'Standard Plan' : 'Premium Plan'}
                            </h2>
                            <p className="text-sm text-gray-600">
                                {webhooks.length} of {getWebhookLimit()} webhooks used
                            </p>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="text-sm text-gray-600">
                                <Clock className="w-4 h-4 inline mr-1" />
                                {getLogRetention()} log retention
                            </div>
                ...
                {activeTab === 'webhooks' ? (
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="space-y-4">
                            ...
                            {webhooks.length >= getWebhookLimit() && user?.userMode !== 'premium' && renderUpgradePrompt()}
                        </div>
                    </div>
                ) : (
                    <div className="bg-white rounded-lg shadow p-6">
